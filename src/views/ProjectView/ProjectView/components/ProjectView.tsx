    ...
    const handleClick = () => {
        if (data.exists) {
            if (data.type === 'quotation') {
                ...
                navigate(`/app/expense-tracker/${id}`)
            } else {
    ...
    useEffect(() => {
        ...
        fetchProject(id)
            .then((data) => {
                ...
                const hasExpenseTracker = !!data?.data?.expenseTrackerId
                ...
                // Add Expense Tracker document
                if (['lpo_received', 'work_started', 'in_progress', 'work_completed', 'invoice_sent', 'team_assigned'].includes(status)) {
                    statusDocuments.push({
                        type: 'expenseTracker',
                        title: 'Expense Tracker',
                        route: `/app/expense-tracker/${id}`,
                        viewRoute: `/app/expense-tracker/${id}`,
